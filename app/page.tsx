...
import { 
  ...
  ScatterChart, Scatter, ZAxis, LineChart, Line, PieChart, Pie, Cell, ComposedChart, Funnel, FunnelChart, LabelList
} from 'recharts';
import { 
...
interface Task { id: number; pulse: string; content: string; source: string; status: 'pool' | 'approved' | 'active' | 'done'; }
...
export default function Dashboard() {
  ...
  const generateAutoInsight = (mergedData: any[]) => {
      ...
      let maxDropMonth = mergedData[0]; // 找反差最大：流量高但轉換低
      let maxDropVal = -1;

      mergedData.forEach(d => {
          if((d.active_users || 0) > (maxTrafficMonth.active_users || 0)) maxTrafficMonth = d;
          if((d.appointments || 0) > (maxAppMonth.appointments || 0)) maxAppMonth = d;
          if((d.total_revenue || 0) > (maxRevMonth.total_revenue || 0)) maxRevMonth = d;

          // 反差指標：流量很大，但每萬人帶來的預約數極低
          if(d.active_users > 5000) {
              const dropRatio = d.active_users / ((d.appointments || 0) + (d.leads || 0) + 1); 
...
🎯 流量巔峰落在 ${maxTrafficMonth.year_month} (${maxTrafficMonth.active_users.toLocaleString()}人)；但預約轉換最優落在 ${maxAppMonth.year_month} (${maxAppMonth.appointments}組)。
💰 營收高點為 ${maxRevMonth.year_month} ($${(maxRevMonth.total_revenue/10000).toFixed(0)}萬)。
⚠️ 需要注意的是 ${maxDropMonth.year_month}，該月雖然有 ${maxDropMonth.active_users.toLocaleString()} 人造訪，卻僅帶來 ${(maxDropMonth.appointments || 0) + (maxDropMonth.leads || 0)} 筆留單，漏水率(反差)最大，建議檢視該月導流的受眾精準度或落地頁動線。`;
  ...
  const refreshData = async (clientName: string) => {
    ...
    try {
        ...
        generateAutoInsight(mergedTrend); // 產生洞察文字

        // 2. 拆分 O2O(婚禮/彌月) 與 EC(節慶) 漏斗資料
        ...
        gaRawItems.forEach(item => {
            ...
            } else if (cat.includes('節慶')) {
                f_traffic += users;
                f_leads += Number(item.leads) || 0;
            } else {
                // 若無明確分類，暫均攤或列入未分類 (此處簡化處理)
                w_traffic += (users / 2); f_traffic += (users / 2);
            }
        });
        ...
        // 估算：將總成交客按預約與名單比例拆分至兩個漏斗 (實務上若有 channel 欄位可精準拆)
        const apptWeight = w_appt / (w_appt + f_leads || 1);
        ...
        setFunnelDataWedding([
            ...
            { name: '4. 大戶回購', value: w_repeat, fill: '#ef4444' }
        ].filter(v => v.value > 0));
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 space-y-8">
                        ...
                        <div className="flex-1 min-h-[300px]">
                            <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                                <ComposedChart data={mergedMonthlyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                    ...
                                    {/* 右 Y 軸：預約與名單 (百位數) - 獨立比例尺，讓曲線明顯跳出來 */}
        ...
        {/* === Page 2: Deep Analysis (V11.0 真漏斗圖) === */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                    ...
                    <div className="flex-1 min-h-[300px] relative">
                        {/* Custom Recharts Funnel with Tooltip for conversion rates */}
                        {((funnelMode === 'wedding' && funnelDataWedding.length > 0) || (funnelMode === 'festival' && funnelDataFestival.length > 0)) ? (
                            <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                                <FunnelChart>
                                    <Tooltip content={<CustomFunnelTooltip />} />
                                    <Funnel
                                        dataKey="value"
                                        data={funnelMode === 'wedding' ? funnelDataWedding : funnelDataFestival}
                                        isAnimationActive
                                    >
                                        <LabelList position="right" fill="#475569" stroke="none" dataKey="name" fontSize={14} fontWeight="bold" />
                                        <LabelList position="center" fill="#fff" stroke="none" dataKey="value" formatter={(val:any) => val.toLocaleString()} fontSize={16} fontWeight="bold" />
                                    </Funnel>
                                </FunnelChart>
                            </ResponsiveContainer>
                        ) : <EmptyState message="缺乏該模式之 GA 與 TX 對應資料" />}
                    </div>
                </div>
        ...
        {activeTab === 'page5' && (
            <div className="space-y-8 animate-in fade-in">
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                    <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-2"><BookOpen className="text-blue-600"/> 數據定義與計算公式 (V11.0 商業邏輯升級版)</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <ul className="space-y-4">
                                ...
                                <SpecItem title="漏水率診斷 (Drop-off Rate)" logic="1 - (下一層數量 / 上一層數量)" desc="在漏斗圖中懸停滑鼠可見，精確找出客戶流失斷點。" />
                            </ul>
                        </div>
                    </div>
                </div>
...
// 客製化漏斗 Tooltip：計算轉換率與漏水率
const CustomFunnelTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    const data = payload[0].payload;
    const topValue = payload[0].chartType?.data?.[0]?.value || data.value; // 最上層數值
    
    // 如果不是第一層，嘗試計算與上一層的差異 (此處簡化為只算與最上層的總轉換率)
    const convRate = topValue > 0 ? ((data.value / topValue) * 100).toFixed(1) : 0;
    
    return (
      <div className="bg-white p-3 border border-slate-200 shadow-xl rounded-lg">
        <p className="font-bold text-slate-800 mb-1">{data.name}</p>
        <p className="text-sm text-slate-600">數量: <span className="font-bold text-slate-900">{data.value.toLocaleString()}</span></p>
        {data.name.includes('1.') ? null : (
            <>
            <p className="text-sm text-green-600 mt-1">總轉換率: {convRate}%</p>
            <p className="text-xs text-red-500">整體漏水率: -{(100 - Number(convRate)).toFixed(1)}%</p>
            </>
        )}
      </div>
    );
...
function PulseCard({ title, icon, color, data, missingMsg, trend }: any) {
    ...
    const colors: any = { blue: { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200' }, green: { bg: 'bg-green-50', text: 'text-green-700', border: 'border-green-200' }, slate: { bg: 'bg-slate-50', text: 'text-slate-700', border: 'border-slate-200' }, yellow: { bg: 'bg-yellow-50', text: 'text-yellow-700', border: 'border-yellow-200' }, red: { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-200' }, purple: { bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200' } };
    ...
    return (
        <div className={`relative p-6 rounded-2xl border-2 transition-all ${hasData ? `bg-white ${c.border} shadow-sm hover:shadow-md` : 'bg-slate-50 border-dashed border-slate-200 grayscale opacity-70'}`}>
            <div className="flex justify-between items-start mb-4"><div className={`flex items-center gap-2 ${c.text}`}><div className={`p-2 rounded-lg ${c.bg}`}>{icon}</div><h3 className="font-bold">{title}</h3></div>{hasData && trend !== null && trend !== undefined && (<span className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full ${trend >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{trend >= 0 ? <TrendingUp size={12}/> : <TrendingDown size={12}/>}{Math.abs(trend)}%</span>)}</div>
            {hasData ? (<div><div className="text-3xl font-black text-slate-800">{data.unit === '$' ? '$' : ''}{Number(data.value).toLocaleString(undefined, { maximumFractionDigits: 2 })}<span className="text-base text-slate-500 font-normal ml-1">{data.unit !== '$' ? data.unit : ''}</span></div></div>) : (<div className="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 p-3 rounded-lg border border-amber-100"><AlertTriangle size={16} className="shrink-0" /><span>{missingMsg}</span></div>)}
        </div>
    );
}
...
function TableRow({ title, val, unit, status, msg }: any) {
    return (
        <tr className="border-b border-gray-100 hover:bg-slate-50">
            ...
            <td className="p-4 font-bold text-slate-900">{val !== null ? `${unit === '$' ? '$' : ''}${Number(val).toLocaleString(undefined, {maximumFractionDigits:2})}${unit !== '$' ? unit : ''}` : '-'}</td>
            <td className="p-4">{status === 'good' && <span className="text-green-600 flex items-center gap-1"><CheckCircle size={16}/> 齊全</span>}{status === 'none' && <span className="text-slate-300">-</span>}</td>
            <td className="p-4 text-xs text-slate-500">{status === 'none' ? <span className="text-amber-500 flex items-center gap-1"><AlertTriangle size={12}/> {msg}</span> : msg}</td>
        </tr>
    );
}

function AiDiagnosisPanel({ clientName, revenue, rfmSegments, nesData, topProducts }: any) { 
    const [d, setD] = useState(""); const [l, setL] = useState(false); 
    const run = async () => { 
        if (!clientName) return; setL(true); 
        try {
    ...
    return (
        <div className="lg:col-span-1 bg-[#1e293b] text-white rounded-2xl p-6 flex flex-col shadow-xl">
            ...
            <div className="flex-1 space-y-4">{d ? <div className="bg-white/10 p-4 rounded-xl text-sm leading-relaxed border border-white/10 animate-in fade-in whitespace-pre-wrap">{d}</div> : <div className="text-slate-400 text-sm text-center py-10">{l ? "AI 正在分析..." : `點擊分析`}</div>}</div>
...
function ConsultantPrescriptionPage({ clientName, rfmSegments, nesData }: any) {
    const [tasks, setTasks] = useState<Task[]>([]); const [newTaskContent, setNewTaskContent] = useState(''); const [newPulse, setNewPulse] = useState('Traffic');
    useEffect(() => {
        if (!rfmSegments || rfmSegments.length === 0) return;
        const generatedTasks: Task[] = [];
        const sleepingWhales = rfmSegments.find((s:any) => s.name.includes('沉睡大戶')); if (sleepingWhales && sleepingWhales.count > 0) generatedTasks.push({ id: 1, pulse: 'Retention', content: `【緊急挽回】致電關心 ${sleepingWhales.count} 位沉睡大戶，發送高單價尊榮回歸禮。`, source: 'System', status: 'pool' });
        const stars = rfmSegments.find((s:any) => s.name.includes('潛力新星')); if (stars && stars.count > 0) generatedTasks.push({ id: 2, pulse: 'VIP', content: `【回購誘發】發送體驗券給 ${stars.count} 位潛力新星。`, source: 'System', status: 'approved' });
    ...
    const poolTasks = tasks.filter(t => t.status === 'pool'); const approvedTasks = tasks.filter(t => t.status === 'approved'); const activeTasks = tasks.filter(t => t.status === 'active');
    return (
...
function DataUploader({ supabase, onSuccess }: any) { 
    const [uploading, setUploading] = useState(false); const [clientName, setClientName] = useState(""); const [uploadType, setUploadType] = useState('tx'); const [encoding, setEncoding] = useState('UTF-8'); const [msg, setMsg] = useState(""); const [errorDetails, setErrorDetails] = useState(""); 
    const handleFile = (e: any) => { 
        setErrorDetails(""); if (!clientName.trim()) { alert("請先輸入客戶名稱！"); e.target.value = ''; return; }
        const file = e.target.files[0]; if (!file) return; setUploading(true); setMsg(`以 ${encoding} 編碼解析 ${uploadType === 'tx' ? '交易' : 'GA'} 資料...`); 
        Papa.parse(file, { header: true, skipEmptyLines: true, encoding: encoding, transformHeader: (header) => header.replace(/^\uFEFF/, '').trim(), complete: async (results) => { 
                let cleanRows: any[] = [];
                if (uploadType === 'tx') {
                    cleanRows = results.data.map((row: any) => { const rawAmount = row['金額'] || row['amount'] || '0'; const amount = parseFloat(rawAmount.toString().replace(/[\s,]/g, '')); let dateStr = row['購買日期'] || row['order_date']; const orderDate = new Date(dateStr); return { order_date: isNaN(orderDate.getTime()) ? new Date() : orderDate, customer_id: row['客戶編號'] || row['customer_id'], amount: isNaN(amount) ? 0 : amount, product_name: row['購買品項'] || row['product_name'], channel: row['通路'] || row['channel'] || 'EC', client_name: clientName, user_id: null }; }).filter((r:any) => !isNaN(r.amount) && r.customer_id); 
                } else if (uploadType === 'ga') {
                    cleanRows = results.data.map((row: any) => { const periodRaw = row['期間'] || ''; let yearMonth = ''; let category = 'Uncategorized'; const match = periodRaw.match(/(\d{4})\.(\d{1,2})月_(.*)/); if (match) { const y = match[1]; const m = match[2].padStart(2, '0'); yearMonth = `${y}-${m}`; category = match[3]; } const cleanNum = (val: string) => parseInt((val || '0').toString().replace(/,/g, ''), 10) || 0; return { client_name: clientName, year_month: yearMonth, category: category, sessions: cleanNum(row['工作階段(互動數)']), active_users: cleanNum(row['活躍使用者']), new_users: cleanNum(row['新使用者']), appointments: cleanNum(row['當月預約數_婚禮/彌月']), leads: cleanNum(row['當月名單數_節慶']), marketing_spend: 0 }; }).filter((r:any) => r.year_month !== '');
                }
                if (cleanRows.length === 0) {
                    const sampleHeader = Object.keys(results.data[0] || {}).join(', '); const isGarbled = sampleHeader.includes(''); const debugInfo = `解析失敗！\n\n【系統讀取到的欄位名稱】：\n${sampleHeader}\n\n【系統讀取到的第一筆資料】：\n${JSON.stringify(results.data[0], null, 2)}`; setErrorDetails(debugInfo); setUploading(false); setMsg(""); e.target.value = ''; return;
                }
                setMsg(`寫入 ${cleanRows.length} 筆資料...`); const BATCH_SIZE = 1000; 
                try { const targetTable = uploadType === 'tx' ? 'transactions' : 'ga_analytics'; for (let i = 0; i < cleanRows.length; i += BATCH_SIZE) { const { error } = await supabase.from(targetTable).insert(cleanRows.slice(i, i + BATCH_SIZE)); if(error) throw error; } setMsg("🎉 上傳成功！"); setUploading(false); e.target.value = ''; onSuccess(clientName); } catch (error: any) { console.error(error); setErrorDetails(`寫入失敗：\n${error.message}`); setUploading(false); setMsg(""); e.target.value = ''; }
            } }); 
    }; 
    return (
        <div className="space-y-6"><input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="請輸入客戶名稱" className="w-full border p-3 rounded-xl focus:ring-2 outline-none"/><div className="flex gap-4"><button onClick={() => {setUploadType('tx'); setErrorDetails("");}} className={`flex-1 py-3 rounded-xl font-bold transition-all ${uploadType === 'tx' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>1. 交易訂單資料</button><button onClick={() => {setUploadType('ga'); setErrorDetails("");}} className={`flex-1 py-3 rounded-xl font-bold transition-all ${uploadType === 'ga' ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-500'}`}>2. GA 流量資料</button></div><div className="bg-slate-50 p-4 rounded-xl border flex items-center justify-between"><span className="text-sm font-bold text-slate-600">📂 檔案編碼</span><select value={encoding} onChange={(e) => setEncoding(e.target.value)} className="border rounded px-3 py-1 text-sm bg-white" disabled={uploading}><option value="UTF-8">UTF-8</option><option value="Big5">Big5</option></select></div><div className={`border-2 border-dashed p-10 text-center cursor-pointer rounded-2xl relative ${uploadType === 'tx' ? 'border-blue-300 bg-blue-50' : 'border-green-300 bg-green-50'}`}><input type="file" accept=".csv" onChange={handleFile} className="absolute inset-0 opacity-0 cursor-pointer" disabled={uploading} /><div className="flex flex-col items-center gap-3">{uploading ? <Loader2 className="animate-spin w-10 h-10 text-slate-400"/> : <FileUp size={48} className={uploadType === 'tx' ? 'text-blue-500' : 'text-green-500'}/>}<span className="font-bold text-slate-700 text-lg">{uploading ? msg : `點擊上傳 CSV`}</span></div></div>{errorDetails && (<div className="mt-4 p-4 bg-red-50 border rounded-xl text-left"><div className="flex items-center gap-2 text-red-700 font-bold mb-3"><AlertTriangle size={18} /> 上傳失敗</div><textarea readOnly className="w-full h-48 p-3 text-sm text-red-600 bg-white border rounded-lg font-mono" value={errorDetails} /></div>)}</div>
    ); 
}

function TabButton({ id, label, icon, active, onClick, isNew }: any) { return <button onClick={onClick} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-sm font-medium ${active ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100'}`}>{icon} {label}</button>; }
